...
def render_update_log(
    ...
    # "Served by" shows which source in the chain actually produced the data; a
    # ⚠ marks a fall-back (primary failed, a backup served) — an early warning
    # that the primary feed is degrading before it fully breaks.
    lines.append("## Freshness (latest stored data point per active series)")
    lines.append("")
    lines.append("| Ticker | Primary | Served by | Freq | Latest | Status |")
    lines.append("|--------|---------|-----------|------|--------|--------|")
    ...
    for s in sorted(series_health, key=_sort_key):
        ...
        served = s.get("served_by")
        served_str = "—" if not served else (f"{served} ⚠" if s.get("fell_back") else served)
        lines.append(
            f"| {s['ticker']} | {s.get('source','?')} | {served_str} | {freq} | "
            f"{ld_str} | {ic} {s['status']} |"
        )
    lines.append("")
    lines.append("---")
    lines.append("*Auto-generated by `market_data.py daily`. "
                 ...
                 "Served-by ⚠ = a fall-back source served (primary degrading).*")
    lines.append("")
    return "\n".join(lines)
